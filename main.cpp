...
//A partial square is a structure as a sort of promise that we're building a square in that place.
// E.g a PartialSquare could be a 3x3 square, then size = 3 and curl_line would be either 1, 2 or 3.
...
Special cases:
- First line (top most) is represented by a root node whose prevLine is line 0's initial state, line_nr = 0
   and whose children are all possible states for line 0 itself
...
//void calculatePossibilities(int line, Matrix adj,int biggestSquare,vector<int> matrixLine){
...
void getPossibleStates(Node *node, vector<int> *line) {
    ...
    //For every empty square or set of squares, if we can choose placing a square,
    //we can also choose NOT placing said square. for every choice we get,
    //we have to duplicate said vector and call this function again.
    uint prevLineSize = node->prevLine.size();
    uint lineSize = line->size(); 
    uint max_length = line->size() > node->prevLine.size() ? line->size() : node->prevLine.size();
...
int main() {
    ...
    Matrix matrix (lines, vector<int>(cols, EMPTY_SQ));
    cout << "DEBUG:The matrix is: " << endl;
    //Init matrix values
